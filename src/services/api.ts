...
export const productAPI = {
  ...
  searchProducts: async (query: string): Promise<Product[]> => {
    ...
    const url = `${API_BASE_URL}/products?search=${encodeURIComponent(sanitizedQuery)}`;
    logger.info('API Request - Search Products', { url, method: 'GET', query: sanitizedQuery });
    
    const response = await fetch(url);
    const data = await response.json();
    
    logger.info('API Response - Search Products', {
      status: response.status,
      statusText: response.statusText,
      response: data
    });
